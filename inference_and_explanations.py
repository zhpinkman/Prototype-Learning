...
def main(args):
    ...
    # NOTE: note that the structure of the adversarial_datasets is actually a dictionary with the name of
    # the dataset as the key and the dataset as the value
    adversarial_datasets = utils.load_adv_data(
        dataset_info=dataset_info, data_dir=args.data_dir, tokenizer=tokenizer
    )

    best_protos_per_adv = {}
    for adv_name, adv_dataset in adversarial_datasets.items():
        best_protos_per_adv[adv_name] = utils.get_best_k_protos_for_batch(
    ...
    bestk_train_data_per_proto = utils.get_bestk_train_data_for_every_proto(
        dataset_info, train_dataset, model_new=model, top_k=5
    )
    ...
    joblib.dump(
        bestk_train_data_per_proto, "artifacts/bestk_train_data_per_proto.joblib"
    )
    ...
    joblib.dump(best_protos_per_adv, "artifacts/best_protos_per_adv.joblib")
    torch.save(model.prototypes.detach().cpu().numpy(), "artifacts/all_protos.pt")
