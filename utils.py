...
class EarlyStopping(object):
    ...
    def activate(self,s1):
        if not self.activated and s1>0: self.activated=True
...
def get_best_k_protos_for_batch(dataset, specific_label, tokenizer, model_new=None,model_path=None,
...
def print_protos(train_dataset, tokenizer, train_ls, which_protos, protos_train_table):
    ...
    for proto_index in which_protos:
        ...
        for index, (sent, y) in enumerate(zip(sents,[train_ls[j] for j in concerned_idxs])):
            if index == 0:
